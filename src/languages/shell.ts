...
function collectShellLexicalRanges(code: string) {
  const heredocs: Array<TokenRange> = []
  collectHeredocs(code, heredocs)
  const ranges = [...heredocs]
  let heredocIndex = 0
  let index = 0

  while (index < code.length) {
    while (heredocIndex < heredocs.length && heredocs[heredocIndex].end <= index) {
      heredocIndex++
    }
    const heredoc = heredocs[heredocIndex]
    if (heredoc && heredoc.start <= index) {
      index = heredoc.end
      continue
    }
...
function collectHeredocs(code: string, ranges: Array<TokenRange>) {
  ...
  while ((match = regex.exec(code))) {
    ...
    const close = new RegExp(`^\\t*${match[2]}\\s*$`, 'm')
